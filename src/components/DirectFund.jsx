...
const DirectFund = ({ id }) => {
  ...
  return (
      <div>
      <button  className="bg-gradient-to-r from-[#6AFEB0] to-[#5CE3FB] rounded-lg px-4 py-2 text-[#111012] mr-4  text-[18px] w-[100%]" onClick={handleOpen}>Donate</button>
    <Modal
        open={open}
        onClose={handleClose}
      ...
      >
        ...
        <input type="text"  value={id}  className="text-white rounded-lg w-[100%] p-4 bg-[#ffffff23] border border-white/50 backdrop-blur-lg mb-4 outline-none hidden" readonly/>
          ...
          <button  className="bg-gradient-to-r from-[#6AFEB0] to-[#5CE3FB] rounded-lg px-4 py-2 text-[#111012] mr-4 lg:text-[20px] md:text-[20px] text-[18px] w-[100%]" onClick={handleDirectFund}>Donate &rarr;</button>
        </Box>
      </Modal>
